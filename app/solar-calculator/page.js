...
export default function SolarCalculator() {
  ...
  return (
    <main className="simple-page calculator-page">
      <nav className="nav shell"><Link className="brand" href="/" aria-label="Galade Technologies home"><span className="brand-mark">G</span><span>GALADE<br /><small>TECHNOLOGIES</small></span></Link><div className="nav-links"><Link href="/">Home</Link><Link href="/products">Products</Link><Link href="/solar-calculator">Solar calculator</Link><Link href="/about">About</Link><Link href="/contact">Contact</Link></div><Link className="nav-cta" href="/contact">Request an estimate <span>↗</span></Link><button className={`menu-toggle ${menuOpen ? "is-open" : ""}`} type="button" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}><span /><span /></button></nav>
      <div className={`mobile-menu ${menuOpen ? "is-open" : ""}`} aria-hidden={!menuOpen}><div className="mobile-menu-links"><Link href="/" onClick={() => setMenuOpen(false)}>Home <span>↗</span></Link><Link href="/products" onClick={() => setMenuOpen(false)}>Products <span>↗</span></Link><Link href="/solar-calculator" onClick={() => setMenuOpen(false)}>Solar calculator <span>↗</span></Link><Link href="/about">About <span>↗</span></Link><Link href="/contact">Contact <span>↗</span></Link></div><p>Professional solar installation, beautifully simple.</p></div>
